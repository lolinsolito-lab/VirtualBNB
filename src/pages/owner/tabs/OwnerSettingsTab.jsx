...
import { Save, User, Building2, AlertTriangle } from 'lucide-react'
...
export default function OwnerSettingsTab() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Notifica l'admin se l'IBAN è cambiato e non è vuoto
      if (formData.iban !== initialIban && formData.iban.trim() !== '') {
        try {
          await fetch('/api/notifyAdmin', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ownerName: formData.full_name || email,
              ownerEmail: email,
              oldIban: initialIban,
              newIban: formData.iban
            })
          })
        } catch (notifyErr) {
          console.error("Errore nell'invio della notifica all'admin:", notifyErr)
          // Non blocchiamo l'utente se l'email fallisce
        }
      }

      setInitialIban(formData.iban)
      setMessage({ text: 'Dati aggiornati con successo!', type: 'success' })
    } catch (err) {
  ...
  if (loading) return <div className="text-dark-200">Caricamento impostazioni...</div>

  return (
    <div className="max-w-4xl pb-20">
      <div className="mb-8">
        <h2 className="font-serif text-[24px] text-white">I Tuoi Dati</h2>
        <p className="font-sans text-[14px] text-dark-200 mt-2">
          Gestisci i dati fiscali e le coordinate bancarie per ricevere i bonifici dei tuoi rendiconti.
        </p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-dark-200 mb-2">Nome Completo</label>
              <input
                type="text"
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-6">
          <h2 className="font-serif text-[18px] text-gold-500 mb-6 flex items-center gap-2">
            <Building2 size={18} /> Dati Fiscali e Coordinate Bancarie
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              ...
              <input
                ...
                className="w-full bg-dark-900 border border-dark-700 rounded-md px-4 py-3 text-white focus:outline-none focus:border-gold-500 transition-colors"
                placeholder="Lascia vuoto se usi il Nome Completo"
              />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              <label className="block text-[11px] uppercase tracking-widest text-dark-200 mb-2 flex items-center justify-between">
                IBAN
                {formData.iban !== initialIban && (
                  <span className="text-gold-500 flex items-center gap-1 normal-case tracking-normal">
                    <AlertTriangle size={12} /> La Direzione verrà notificata del cambio
                  </span>
                )}
              </label>
              <input
